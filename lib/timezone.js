...
// The reverse of zonedTimeToUtc: given a UTC Date and a timezone, returns the
// wall-clock date/hour/minute that instant represents in that timezone. Used
// to compute the "same time tomorrow/next week" for recurring posts.
export function utcToZonedParts(utcDate, timeZone) {
  const fmt = new Intl.DateTimeFormat("en-US", {
    timeZone,
    hour12: false,
    year: "numeric",
  ...
  return {
    ...
    hour24: parts.hour === "24" ? 0 : Number(parts.hour),
    minute: Number(parts.minute),
  };
}

// Given a post's current scheduledFor (UTC) and the timezone it was created in,
// returns the next UTC instant for a daily or weekly repeat.
export function nextOccurrence(currentUtcDate, timeZone, repeat) {
  const { dateStr, hour24, minute } = utcToZonedParts(currentUtcDate, timeZone);
  const [y, m, d] = dateStr.split("-").map(Number);
  const daysToAdd = repeat === "weekly" ? 7 : 1;
  const nextDate = new Date(Date.UTC(y, m - 1, d + daysToAdd));
  const nextDateStr = nextDate.toISOString().slice(0, 10);
  return zonedTimeToUtc(nextDateStr, hour24, minute, timeZone);
}
