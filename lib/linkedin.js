...
export async function publishLinkedInPost({ accessToken, authorSub, text, imageUrns }) {
  const images = (imageUrns || []).filter(Boolean);
  console.log(`Publishing post. Commentary length: ${text.length} chars. Images: ${images.length}`);
  const body = {
  ...
  if (images.length === 1) {
    ...
    // LinkedIn supports up to 9 images in a single carousel post.
    body.content = {
      multiImage: {
        images: images.slice(0, 9).map((id) => ({ id })),
      },
    };
  }
